...
```{r setup, echo = TRUE, warning. = FALSE, messge = FALSE}
...
renderPlot({
  ggplot(data = penguin_set(), aes(x = flipper_length_mm, y = body_mass_g, color = species)) +
    geom_point(size = input$pt_size) +
    scale_color_manual(breaks = c('Adelie', 'Chinstrap', 'Gentoo'),
                       values = c('orange', 'green', 'blue')) +
    scale_x_continuous(limits = range(penguins$flipper_length_mm, #this is maintaining the axis the same per species
                                      na.rm = TRUE)) +
    scale_y_continuous(limits = range(penguins$body_mass_g,
                                      na.rm = TRUE))
})
```
